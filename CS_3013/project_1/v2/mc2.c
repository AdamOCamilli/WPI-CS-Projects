...
typedef struct {
  int ids[60]; // pids
  struct timeval times[60]; // starting times of pids
} Background;
...
void execute(char* cmd) {
  ...
  int* status;
  int background = 0; // is a background process
  int num_bgs = 0; // number of background processes running
  Background bg;
  ...
  while ((tok = strtok(new_cmd_copy, " ")) != NULL) {
    ...
    } else { background = 1; } // if ampersand detected, command runs in background
    //printf("Token: %s\n",tok);
    new_cmd_copy = NULL;
  }

  //printf("Second to last: %s\n",exec_str[arg_index-1]);
  ...
  if (pid < 0) { // bad fork
    ...
    execvp(exec_str[0],exec_str); // shouldn't return
    fprintf(stderr, "Invalid Command\n");
  } else { // parent
     if (background) {
      bg.ids[num_bgs++] = getpid(); // Keep track of pid of this background process
      bg.times[num_bgs] = start; // keep track of starttime of this background process
      printf("[%d]: %d\n",num_bgs,bg.ids[num_bgs - 1]);
      pid_t wait_status;
      int i;
