...
drop procedure NumberOfStaff;

create procedure NumberOfStaff (officeID IN varchar2) is
...
Begin
    ...
    for staff in C_Staff loop
        if staff.officeID = officeID
            then total_staff := total_staff + 1;
        end if;
    end loop;
    
    dbms_output.put_line(total_staff);
...
Begin
    ...
    if(numOfTitles > 3)
        then RAISE_APPLICATION_ERROR(-20001, 'Professor has more than three title');
    end if;
