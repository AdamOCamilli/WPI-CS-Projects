...
def cosine_similarity(RA, RB):
    ...
    AB, AA, BB = 0,0,0
    for i in range(len(RA_both)): # arbitrary, they are equal
        AB += RA_both[i] * RB_both[i]
        AA += RA_both[i] ** 2
        BB += RB_both[i] ** 2
...
def user_similarity(R, j, idx):
    ...
    sim = []
    for i in idx:
        Ri = [] # Ratings of user in idx
        for r in R:
            Ri.append(r[i])
        # print("Ri:",Ri)
        sim.append(float(cosine_similarity(Rj,Ri))) # math.sqrt returns float64
...
def user_based_prediction(R, i_movie, j_user, K=5):
    ...
    j_similarities = user_similarity(R,j_user,i_users)
    

    # compute the weighted average of the top K similar users to user j

    j_similarities_sorted = sorted(j_similarities,reverse=True)

    num,denom = 0.,0.
    for x in range(K):
        orig_index = j_similarities.index(j_similarities_sorted[x])
        orig_rating = R[i_movie][i_users[orig_index]]
        num += j_similarities_sorted[x]*orig_rating
        denom += j_similarities_sorted[x]
...
def compute_RMSE(ratings_pred, ratings_real):
    ...
    sum = 0.
    for p,r in zip(ratings_pred, ratings_real):
        sum += (p - r) ** 2)
    
    RMSE = math.sqrt(sum / np.mean(ratings_pred))
...
def load_rating_matrix(filename = 'movielens_train.csv'):
    ...
    # Build array (need to know max row + max col)
    f = open(filename, "rb")
    cols = np.genfromtxt(f, delimiter=",", dtype=int, usecols=0)
    f.close() # Need to close file after each genfromtxt()
    
    f = open(filename, "rb")
    rows = np.genfromtxt(f, delimiter=",", dtype=int, usecols=1)
    f.close()        
    ...
    # Populate array
    f = open(filename, "rb")
    lines = np.genfromtxt(f, delimiter=",", dtype=int)
    f.close()
...
def load_test_data(filename = 'movielens_test.csv'):
    ...
    f = open(filename, "rb")
    ratings = np.genfromtxt(f, delimiter=",", dtype=float, usecols=2).tolist()
    f.close()

    m_ids, u_ids = [], []
    for c in cols:
        u_ids.append(int(c - 1))
    for r in rows:
        m_ids.append(int(r - 1))
...
def movielens_user_based(train_file='movielens_train.csv', test_file ='movielens_test.csv', K = 5):
    ...
    # pairs = list(zip(m_ids, u_ids))
    # for i,j in pairs:
    #     ratings_pred.append(user_based_prediction(R,i,j,K))
    ratings_pred = (user_based_prediction(R, m_ids[i], u_ids[i]) for i in range(len(ratings_real)))
