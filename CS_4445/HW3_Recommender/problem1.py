...
def eval_player(ID='hattesc01'):
    ...
    p = df.loc[df["playerID"] == ID]
    i = p.index.values

    BA = batting_average(df.iloc[i]["H"], df.iloc[i]["AB"])
    OBP = on_base_percentage(df.iloc[i]["H"], df.iloc[i]["AB"], df.iloc[i]["BB"], df.iloc[i]["HBP"], df.iloc[i]["SF"])
    SLG = slugging_percentage(df.iloc[i]["H"], df.iloc[i]["2B"], df.iloc[i]["3B"], df.iloc[i]["HR"], df.iloc[i]["AB"])
