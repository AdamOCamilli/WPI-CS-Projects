...
def reshape_to_image(x):
    ...
    x = np.asmatrix(x) # sometimes it's passed as a list
    
    sqrt = int(np.sqrt(x.shape[1]))
    image = x.reshape(sqrt,sqrt)
...
def compute_mu_image(X):
    ...
    mean_vector = np.asmatrix([np.average(v) for v in X.transpose()])
...
def compute_eigen_faces(X, k=20):
    ...
    P_tp = P.transpose()
    P_images = [reshape_to_image(P_tp[i]) for i in range(k)]
...
def load_dataset():
    ...
    y = np.asmatrix(dataset.target)
    y = y.transpose()
    # statistics of the data

    X = np.asmatrix(dataset.data)
