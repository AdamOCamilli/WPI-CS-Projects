...
def compute_dL_da(a, y):
    ...
    if (y == 1):
        if (a == 0):
            dL_da = 1e4 # test requires < 1e5 arbitrarily?
        else:
            dL_da = (1.0-2*y)/a
    else:
        if (a == 1):
            dL_da = 1e6 # use same arbitrary value from previous test requiring > 1e5?
        else:
            dL_da = (1.0-2*y)/(1-a)  # use different derivative

    print(a,y,L,dL_da, check_dL_da(a,y))
    #########################################
    return dL_da 
...
def compute_dL_dw(dL_da, da_dz, dz_dw):
    ...
    dL_dw = np.asmatrix(np.transpose(np.array([[dL_da*da_dz*i[0,0] for i in dz_dw]])))
