...
void client_handler(int csock, int ssock) {
  ...
  int total = 0;
  while ((read_size = recv(csock, client_message, MAX_MSG_LEN, 0) != 0) && run) {
    //printf("Read size = %d\n",read_size);;
    if (read_size < 0) {
      perror("recv");
      printf("errno: %d\n",errno);
	...
	hash_len = strlen(search_by_socket(usertable,csock)->key);
	reply = (search_by_name(usertable, client_message))->salt; // Send user their salt
	converted_reply = htonl(reply);
	printf("Salt = %ld\n", reply);
	...
	memset(client_message, 0, MAX_MSG_LEN);
	continue;
      } else if (strlen(client_message) >= hash_len) {
	client_message[hash_len] = '\0';
	printf("Client: %s\n",client_message);
	...
	if ((search_by_key(usertable, client_message) != NULL)) { 
      ...
      reply = 0;
      converted_reply = reply;
      printf("Writing %ld back to client\n",converted_reply);
      write(csock, &converted_reply, sizeof (long *));
      ...
      memset(client_message, 0, MAX_MSG_LEN);
      continue;
    } else if (strncmp(client_message, "#", MAX_MSG_LEN) == 0) {
      client_disconnect = 1;
      break;
    } else { // 3. User is sending commands
      // Execute user's command in current working directory
	...
	strcat(temp_message,fbuf);
      ...
      char *dir_buf = malloc(MAX_MSG_LEN);
      free(fbuf);
      if (strcmp(temp_message,"\n") != 0) {
	write(csock, &temp_message, MAX_MSG_LEN);
	fclose(fp);
	memset(client_message, 0, MAX_MSG_LEN);
	continue;
      } else {
	// Always respond so client knows its message has been received
	memset(temp_message,0,MAX_MSG_LEN);
	strncat(temp_message, client_message, MAX_MSG_LEN);
	strcat(temp_message, "\n");
//      printf("Received command \n%s\nfrom %s\n",client_message,search_by_socket(usertable,csock)->name);
  //    printf("Executing command...\n\n");
    //  printf("\nReading output to %s...",search_by_socket(usertable,csock)->name);
	write(csock, &temp_message, MAX_MSG_LEN);
      //printf("%s: %s\n", search_by_socket(usertable,csock)->name, client_message);
	memset(client_message, 0, MAX_MSG_LEN);
	fclose(fp);
